...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  useEffect(() => {
    const hasImage =
      !!localProduct.images &&
      ((typeof localProduct.images === "string" && localProduct.images.trim().length > 0) ||
    ...
    async function fetchDetails() {
      try {
        const url = `${base.replace(/\/$/, "")}/products/${encodeURIComponent(product.slug)}`;
        const res = await axios.get(url, { signal: controller.signal });
        if (res?.data) setLocalProduct((prev) => ({ ...prev, ...res.data }));
      } catch (err: any) {
        if (err?.name === "CanceledError") return;
        if (process.env.NODE_ENV === "development") console.debug("ProductCard fetch failed", err);
      }
    }
  ...
  function prefixIfRelative(url?: string) {
    ...
    const base = process.env.NEXT_PUBLIC_API_BASE || (typeof window !== "undefined" ? window.location.origin : "");
  ...
  function resolveImageUrl(p?: Product) {
    ...
    if (typeof imgs === "string") return imgs.trim() ? prefixIfRelative(imgs) : PLACEHOLDER;
    ...
    <Link href={href} className="group">
      <div className="bg-white rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300 overflow-hidden">
        ...
        <div className="p-4">
          ...
          {localProduct.rating ? (
            <div className="mt-1 text-xs text-gray-600">Rating: {localProduct.rating.toFixed(1)} ★</div>
          ) : null}
        </div>
      </div>
    </Link>
